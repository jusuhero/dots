...
gitmux_icon=$(tmux_get '@tmux_power_gitmux_icon' '󰊢 ')
...
show_gitmux="$(tmux_get @tmux_power_show_gitmux false)"
show_web_reachable="$(tmux_get @tmux_power_show_web_reachable false)"
prefix_highlight_pos=$(tmux_get @tmux_power_prefix_highlight_pos)
time_format=$(tmux_get @tmux_power_time_format '%H:%M')
date_format=$(tmux_get @tmux_power_date_format '%d.%m')
# short for Theme-Colour
TC=$(tmux_get '@tmux_power_theme' 'coral')
case $TC in
    'gold' )
        TC='#ffb86c'
        ;;
...
if "$show_gitmux"; then
    RS="#[fg=$G05,bg=$BG]$larrow#[fg=$TC,bg=$G05] $gitmux_icon #(/home/ju/go/bin/gitmux "#{pane_current_path}") $RS"
fi
